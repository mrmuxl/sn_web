...
def lan_line_chart(request):
    ...
    if not request.user.is_superuser:
        ...
        day_list =[day-datetime.timedelta(days=d) for d in range(29,-1,-1)]
        try:
            lan_list = KxLanDay.objects.filter(tongji_day__range=(old_day,day)).values('id','lan_num','tongji_day')
        except Exception as e:
            lan_list = []
            logger.debug("%s",e)
        lan_line = []
...
def remain_ratio_chart(request):
    ...
    if not request.user.is_superuser:
        ...
        old_day = day-datetime.timedelta(days=29)
        day_list =[day-datetime.timedelta(days=d) for d in range(29,-1,-1)]
        bug_list = KxTongjiRecord.objects.filter(tongji_day__range=(old_day,day)).values('id','cur_mon_num','last_mon_num','tongji_day')
        remain_ratio = []
        if len(bug_list) < 30:
            for i in range(30):
                try:
