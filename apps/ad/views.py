...
def operator_select(request):
    ...
    if email:
        opt = Operator.objects.filter(status__exact=1).filter(expire__gt=now).filter(operatorassistant__user__email=email,operatorassistant__status__exact=1)
        if opt:
            message['status']=0
            message['is_assistant']=True
            message['is_print']=True
            message['dislplay']=u'你可以共享一台打印机'
            message['link']=u'http://www.simplenect.cn/buy'
            message['show']=False
            ...
            message['link']=u'http://www.simplenect.cn/buy'
            message['show']=True #控制 "购买" 按钮是否显示
            return HttpResponse(json.dumps(message,ensure_ascii=False),content_type="application/json")
    else:
        message['status']=1
        return HttpResponse(json.dumps(message,ensure_ascii=False),content_type="application/json")
