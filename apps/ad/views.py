...
def operator_select(request):
    ...
    if email:
        ...
        if owner:
            ...
            message['show_buy_link']=False # @todo 2016-11-11 开放免费版改动 
            return HttpResponse(json.dumps(message,ensure_ascii=False),content_type="application/json")
        else:
            message['status']=0
            message['is_owner']=False
            ...
            message['is_print']=True # @todo 2016-11-11 开放免费版改动 
            #message['dislplay']=u'您目前没有权限共享打印机' # @todo 2016-11-11 开放免费版改动 
            message['dislplay']=u'你可以共享一台打印机'
