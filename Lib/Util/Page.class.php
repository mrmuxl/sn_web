...
class Page extends Think {
    ...
    //protected $config  =	array('header'=>'条记录','prev'=>'上一页','next'=>'下一页','first'=>'第一页','last'=>'最后一页','theme'=>' %totalRow% %header% %nowPage%/%totalPage% 页  %first%  %prePage%  %upPage% %linkPage% %downPage% %nextPage%  %end%');
  	protected $config  =	array('header'=>'条记录','prev'=>'上一页','next'=>'下一页','first'=>'第一页','last'=>'最后一页','theme'=>'<span class="page_total"> %totalRow%%header% 共%totalPage%页</span><ul>%first%  %upPage% %linkPage% %downPage%  %end% </ul>  ');
    /**
     +----------------------------------------------------------
     * 架构函数
     +----------------------------------------------------------
    ...
    public function show() {
        ...
        if ($upRow>0){
            $upPage="<li class='page_li_guide page_prev'><a href='".$url."&".$p."=$upRow'>".$this->config['prev']."</a></li>";
        }else{
            $upPage="";
        }

        if ($downRow <= $this->totalPages){
            $downPage="<li class='page_li_guide page_next'><a href='".$url."&".$p."=$downRow'>".$this->config['next']."</a></li>";
        }else{
            $downPage="";
        }
        // << < > >>
        if($nowCoolPage == 1){
            ...
            $theFirst = "<li class='page_li_guide page_first'><a href='".$url."&".$p."=1' >".$this->config['first']."</a></li>";
        }
        if($nowCoolPage == $this->coolPages){
            ...
            $theEnd = "<li class='page_li_guide page_last'><a href='".$url."&".$p."=$theEndRow' >".$this->config['last']."</a></li>";
        }
        // 1 2 3 4 5
        $linkPage = "";
...
//        }
    ...
    */
    ...
    public function show_more($n) {
        ...
        $downRow = $this->nowPage+1;
        if ($upRow>0){
            $upPage="<li class='page_li_guide page_prev'><a href='javascript:void(0);' id='page_prev'>".$this->config['prev']."</a></li>";
        }else{
            $upPage="";
        }

        if ($downRow <= $this->totalPages){
            $downPage="<li class='page_li_guide page_next'><a href='javascript:void(0);' id='page_next'>".$this->config['next']."</a></li>";
        }else{
            $downPage="";
        }
        // << < > >>
